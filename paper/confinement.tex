...
%(4) Figure 1: you should include the case (vi) as a purple diamond as well. Add it to the figure on the left as well (you'll have to extend the range of dgg to whatever it is graphene-graphene). You should scale the snapshots such that they have the same width, instead of fitting them to the height. It may be better to do a 2-column separate figure of the right panel, with 3 cases in each column. Right now it is really hard to tell what the heat maps represent. Also, you don't need the red arrow with the ro_2d, it is pretty self-explanatory with the snapshots. Another possibility would be to have in the column of the left the diamond cases for flexible graphene, and the column on the right the same rigid cases. You don't need units in the heat maps, they are impossible to read anyway.

%(5) Figure 2: I would add the bulk g(r) and angle probability as a light gray curve in either all the panels or at least the panels on the right. Also, given that we only show 3 curves per panel, could you try to plot the g(r) without the shifts, maybe it doesn't look too busy. I like the shifted curves but it is really hard to see, other than when it is ice, the shape of the curve after the first peak. Finally, you don't need to include the labels of the density in every panel. You can either make a single legend for all the plots or even just tell early the caption what black, red, and blue correspond to.
