...
def plot_scatter(csv, sep, ln, itr):
    ...
    d1 = csv.dat[csv.find_keyword('dis1')].flatten()
    d2 = csv.dat[csv.find_keyword('dis2')].flatten()

    good = np.all(np.array([d1<3.25,d2<3.25]),axis=0)
    angles = angles[good]; ds = np.array([d1[good], d2[good]]).flatten()
    ...
    plt.savefig(csv.csvfname[:-4]+'.png',bbox_inches='tight'); plt.close()
    ...
    f = plt.figure(1, figsize = (1.5, 1.5)); ax = f.add_subplot(111)
    rng = [2, 7]
    y,x = np.histogram(ds, bins=nbins, range=rng); dx = x[1]-x[0]
    x = x[:-1] + dx/2.;y = y/sum(y.astype(float))/dx
    plt.plot(x, y, color = "k")
    ax.set_xlim(rng);#ax.set_ylim([0,0.02])

    ax.set_xlabel(r"Pair distances $\AA$",fontsize= 12)
    ax.set_ylabel(r"PDF",fontsize= 12)
    plt.savefig(csv.csvfname[:-4]+'.dist.png',bbox_inches='tight'); plt.close()
