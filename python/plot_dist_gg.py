...
def plot_scatter(csv, sep, ln, itr):
   ...
   #print(min(Y), max(Y), min(X), max(X))
    ...
    print("Dgg maxes: ", x[argrelextrema(y, np.greater)],
          " Dgg mins: ", x[argrelextrema(y[5:-5], np.less)])
    f = plt.figure(1, figsize = (1.0, 1.0))
    ax, ct, leg = f.add_subplot(111), 0, []
    ax.xaxis.set_major_locator(MaxNLocator())
    ax.yaxis.set_major_locator(MaxNLocator())
   ...
   #print("X position maxes: ", x[argrelextrema(y, np.greater)])
    ...
    y,x = np.histogram(Y, bins=nbins); dx = x[1]-x[0]
    if len(x[argrelextrema(y[5:-5], np.less)]) > 0:
        ml = Y < x[argrelextrema(y[5:-5], np.less)][0]
        bl = Y > x[argrelextrema(y[5:-5], np.less)][0]
        Xml = X[ml]; Xbl = X[bl]
        ...
        print("{0:5} this is n-L/(n+1)-L: {1:.3f}".format(sep,sum(y_ml.astype(float))
                                                   /sum(y_bl.astype(float))))
 
        f = open("run"+sep+"1_46_1.diso_sep_fit.dat", "w")
        f.write("x_dgg,x_val\n")
        for i in range(len(x_ml)): f.write("{0:.5f},{1:.5f}\n".format(x_ml[i],y_ml[i]))
        f.close()
 
        f = open("run"+sep+"2_46_1.diso_sep_fit.dat", "w")
        f.write("x_dgg,x_val\n")
...
def main():
    ...
    csvname = sys.argv[1]; sep = sys.argv[2]; ln = int(sys.argv[3]);
    itr = sys.argv[4]
