...
class ZipFile:
    ...
    def print_coords(self, volC):
        ...
        for ti in range(len(self.xyzC.xyz)):
            if nat == 0: nat = len(self.xyzC.xyz[ti])
            if nat != len(xyzC.xyz[ti]) : print("Different number of atoms")

            resno = 1
            f.write("confined syst, t= {0}\n{1}\n".format(volC.time[ti,1],nat))
            ## "%5d%-5s%5s%5d%8.3f%8.3f%8.3f%8.4f%8.4f%8.4f"
            for i in range(xyzC.nat):
                if xyzC.ty[i] == WOXY or xyzC.ty[i] == GRAPHENE: resno += 1
                strn = "{0:5}{1:>5}{2:5}{3:5}".format(resno,
                       type_dic[xyzC.ty[i]][0], type_dic[xyzC.ty[i]][1], i)

                strn += "{0:8.3f}{1:8.3f}{2:8.3f}".format(*(xyzC.xyz[ti][i][j]))
                strn += "{0:8.4f}{1:8.4f}{2:8.4f}".format(*(velC.xyz[ti][i][j]))
                strn+="\n"
                f.write(strn)
